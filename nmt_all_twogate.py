...
# batch preparation
def prepare_data(seqs_x, seqs_y, seqs_z, maxlen=None,
    ...
    if maxlen is not None:
        ...
        new_lengths_x = []
        new_lengths_y = []
        for l_x, s_x, l_y, s_y, s_z in zip(lengths_x, seqs_x, lengths_y, seqs_y, seqs_z):
    ...
    z = numpy.zeros((maxlen_y, n_samples)).astype('int64')
...
def param_init_gru_cond(options, params, prefix='gru_cond',
    ...
    params[_p(prefix, 'bt')] = numpy.zeros((2 * (dim),)).astype('float32')
    Ut = numpy.concatenate([ortho_weight(dim_nonlin),
                           ortho_weight(dim_nonlin)], axis=1)
    params[_p(prefix, 'Ut')] = Ut

    Wtx = norm_weight(nin, dim)
    params[_p(prefix, 'Wtx')] = Wtx
    Utx = ortho_weight(dim_nonlin)
    params[_p(prefix, 'Utx')] = Utx
    params[_p(prefix, 'btx')] = numpy.zeros((dim,)).astype('float32')
    
    Ut_nl = numpy.concatenate([ortho_weight(dim_nonlin),
                              ortho_weight(dim_nonlin)], axis=1)
    params[_p(prefix, 'Ut_nl')] = Ut_nl
    params[_p(prefix, 'bt_nl')] = numpy.zeros((2 * dim_nonlin,)).astype('float32')

    Utx_nl = ortho_weight(dim_nonlin)
    params[_p(prefix, 'Utx_nl')] = Utx_nl
    params[_p(prefix, 'btx_nl')] = numpy.zeros((dim_nonlin,)).astype('float32')

    # context to LSTM
    Wtc = norm_weight(dimctx, dim*2)
    params[_p(prefix, 'Wtc')] = Wtc

    Wtcx = norm_weight(dimctx, dim)
    params[_p(prefix, 'Wtcx')] = Wtcx

    # attention: combined -> hidden
    Wt_comb_att = norm_weight(dim, dimctx)
    params[_p(prefix, 'Wt_comb_att')] = Wt_comb_att

    # attention: context -> hidden
    Wtc_att = norm_weight(dimctx)
    params[_p(prefix, 'Wtc_att')] = Wtc_att

    # attention: hidden bias
    bt_att = numpy.zeros((dimctx,)).astype('float32')
    params[_p(prefix, 'bt_att')] = bt_att

    # attention:
    Ut_att = norm_weight(dimctx, 1)
    params[_p(prefix, 'Ut_att')] = U_att
    c_att = numpy.zeros((1,)).astype('float32')
    ...
    W3 = numpy.concatenate([ortho_weight(dim_nonlin),
                              ortho_weight(dim_nonlin)], axis=0)
    params[_p(prefix, 'W3')] = W3
    ####################################
    
    return params


def gru_cond_layer(tparams, state_below, z, options, prefix='gru',
    ...
    #########
    pctxz_ = tensor.dot(context, tparams[_p(prefix, 'Wtc_att')]) + \
        tparams[_p(prefix, 'bt_att')]
    #########
    ...
    def _step_slice(m_, x_, xx_, z_, zx, h_, ctx_, alpha_, pctx_, cc_, pctxz_,
                    ...
                    Ut, Wtc, Wt_comb_att, Ut_att, ct_tt, Utx, Wtcx,
                    Ut_nl, Utx_nl, bt_nl, btx_nl,
                    W1, W2, W3):
        ...
        # attention
        pstate_ = tensor.dot(h1, Wt_comb_att)
        pctx__ = pctxz_ + pstate_[None, :, :]
        pctx__ = tensor.tanh(pctx__)
        alpha = tensor.dot(pctx__, Ut_att)+ct_tt
        ...
        preact2 = tensor.dot(h1, Ut_nl)+bt_nl
        preact2 += tensor.dot(ctx_, Wtc)
        preact2 = tensor.nnet.sigmoid(preact2)

        r2 = _slice(preact2, 0, dim)
        u2 = _slice(preact2, 1, dim)

        preactx2 = tensor.dot(h1, Utx_nl)+btx_nl
        preactx2 *= r2
        preactx2 += tensor.dot(ctx_, Wtcx)
    ...
    shared_vars = [tparams[_p(prefix, 'U')],
                   ...
                   tparams[_p(prefix, 'Ut')],
                   tparams[_p(prefix, 'Wtc')],
                   tparams[_p(prefix, 'Wt_comb_att')],
                   tparams[_p(prefix, 'Ut_att')],
                   tparams[_p(prefix, 'ct_tt')],
                   tparams[_p(prefix, 'Utx')],
                   tparams[_p(prefix, 'Wtcx')],
                   tparams[_p(prefix, 'Ut_nl')],
                   tparams[_p(prefix, 'Utx_nl')],
                   tparams[_p(prefix, 'bt_nl')],
                   tparams[_p(prefix, 'btx_nl')],
                   tparams[_p(prefix, 'W1')],
                   tparams[_p(prefix, 'W2')],
                   tparams[_p(prefix, 'W3')],
                   ]

    if one_step:
        rval = _step(*(seqs + [init_state, None, None, pctx_, context, pctxz_] +
                       shared_vars))
    else:
        rval, updates = theano.scan(_step,
                                    sequences=seqs,
                                    outputs_info=[init_state,
                                                  ...
                                                  tensor.alloc(0., n_samples,
                                                               context.shape[0])],
                                    non_sequences=[pctx_, context, pctxz_]+shared_vars,
                                    name=_p(prefix, '_layers'),
                                    n_steps=nsteps,
                                    profile=profile,
                                    strict=True)
...
def build_model(tparams, options):
    ...
    return trng, use_noise, x, x_mask, y, y_mask, z, opt_ret, cost
...
def build_sampler(tparams, options, trng, use_noise):
    ...
    z = tensor.vector('z_sampler', dtype='int64')
...
def gen_sample(tparams, f_init, f_next, x, z, options, trng=None, k=1, maxlen=30,
...
def pred_probs(f_log_probs, prepare_data, options, iterator, worddicts, verbose=False):
    probs = []
    ...
    for x, y, z in iterator:
        ...
        pprobs = f_log_probs(x, x_mask, y, y_mask, z)
        for pp in pprobs:
            probs.append(pp)
...
def prepare_z(y, z):
    ...
    for i in range(len(y)):
        seqy = tuple(y[i])   
        if len(z[i]) != 1:
            print("Mismatch keyword in current y_seq: ", y[i])
            print("Current z is: ", z[i])
            print("Type of current z: ", type(z[i]))
            exit()
        # add all / gate
        cur_list = [z[i][0]] * len(seqy)
        # add one
        # cur_list = [0] * len(seqy)
        # cur_list[0] = z[i][0]
        new_z.append(cur_list)
    return new_z
...
def train(dim_word=100,  # word vector dimensionality
    ...
    f_pplxity = theano.function(inps, pplxity, profile=profile)
    print('Done')
    ...
    for eidx in range(max_epochs):
        ...
        for x, y, z in train:
            ...
            perplexity = f_pplxity(x, x_mask, y, y_mask, z).mean()
            ...
            if numpy.mod(uidx, dispFreq) == 0:
                print 'Epoch ', eidx, 'Update ', uidx, 'Cost ', cost, 'perplexity:', perplexity, 'UD ', ud
